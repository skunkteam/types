...
describe.each(['with', 'without'] as const)('%s BigInt support', bigIntSupport => {
    jest.resetModules();

    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    (global as any)['BigInt'] = bigIntSupport === 'with' ? BigIntConstructor : undefined;
    const { basicTypeMessage, defaultMessage, defaultUsualSuspects, testTypeImpl } =
        jest.requireActual<typeof import('../testutils.js')>('../testutils.js');
    const { humanList } = jest.requireActual<typeof import('../utils/index.js')>('../utils/index.js');
    const { int, number } = jest.requireActual<typeof import('./number.js')>('./number.js');
    ...
    testTypeImpl({
        ...
        invalidValues: [
            ...
            [
                -1.5,
                [
                    'errors in [NonNegativeInt]:',
                    '',
                    '- expected a non-negative number, got: -1.5',
                    '',
    ...
    testTypeImpl({
        ...
        invalidValues: [
            ...
            [
                101,
                [
                    'errors in [SimpleDecade]:',
                    '',
                    '- expected a number less than 100, got: 101',
                    '',
                    '- expected a multiple of 10, got: 101',
                ],
            ],
        ],
    });
    ...
    testTypeImpl({
        ...
        validValues: [
            ...
            ...(bigIntSupport === 'with' ? [Number.MAX_VALUE, Number.MAX_VALUE / 10] : []),
        ],
        invalidValues: [
            -100.111,
            123456789.012,
            ...
            1000000.335,
            123456789.222,
            ...(bigIntSupport === 'without' ? [Number.MAX_VALUE, Number.MAX_VALUE / 10] : []),
        ].map(v => [v, `error in [Currency]: expected a multiple of 0.01, got: ${v}`]),
    });
